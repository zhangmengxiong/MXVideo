...
internal class MXHistoryDB(val context: Context) {
    ...
    fun getPlayTime(videoPath: String): Int {
        ...
        try {
            ...
            if (cursor != null && cursor.moveToFirst()) {
                return cursor.getInt(cursor.getColumnIndexOrThrow(DBHelp.DB_KEY_PLAY_TIME))
            }
        } catch (e: Exception) {
            e.printStackTrace()
        } finally {
    ...
    private class DBHelp(context: Context) : SQLiteOpenHelper(context, DB_NAME, null, 2) {
        companion object {
            const val DB_NAME = "mx_video_db"
            ...
            private const val DB_CREATE =
                "create table $DB_NAME($DB_KEY_VIDEO_PATH text NOT NULL UNIQUE , $DB_KEY_PLAY_TIME long )"
        }
